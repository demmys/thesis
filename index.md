...
4. 提案手法において発生しうる副作用<br>
   本研究ではコンパイラの可読性を向上する目的でSelf-host化を用いるが、他の言語におけるコンパイラではさまざまな目的を持ってSelf-host化が行われており、その影響範囲は大きい
    1. Self-host化の事例
        1. Go - go
        2. Python - PyPy
...
5. Self-host化したSwiftコンパイラ「TreeSwift」の設計と実装 :implementation
    1. 可読性比較可能箇所の絞り込み :comperable
        1. Swiftコンパイラの構造 :structure
            * Swiftコンパイラはおおまかに構文解析、意味解析、SIL(Swift Intermediate Language)生成、詳細解析、IR(LLVM-IR)生成のステップに分けられる
        2. 可読性の比較が可能な箇所 :part
            * :open-source:readabilityで述べたように、扱う問題や手法が異なるとそのプログラムの複雑性は大きく変化する
            * 型推論や型検査を行う意味解析と最適化などを行う詳細解析は同じ問題を対象として同じ手段を取っているということを確認するのが難しい
